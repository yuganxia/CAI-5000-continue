...
namespace CombatAI
{
    public static class TCacheHelper
    {
		...
		private static class Methods
		{
			...
			public static int Bill(Bill bill)
			{
				if (bill == null) return 0;
				Type t = bill.GetType();
				var f = t.GetField("loadID", BindingFlags.Instance | BindingFlags.Public | BindingFlags.NonPublic);
				if (f != null)
				{
					var v = f.GetValue(bill);
					if (v is int i) return i;
				}
				var p = t.GetProperty("loadID", BindingFlags.Instance | BindingFlags.Public | BindingFlags.NonPublic);
				if (p != null)
				{
					var v = p.GetValue(bill);
