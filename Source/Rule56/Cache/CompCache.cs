...
namespace CombatAI
{
    public static class CompCache
    {
        ...
        public static T GetComp_Fast<T>(this ThingWithComps thing, bool allowFallback = true) where T : ThingComp
        {
            ...
            var __comps = CombatAI.Compatibility.ThingWithCompsCompat.GetComps(thing);
            if (__comps == null)
            {
                return null;
            }
            ...
            for (int i = 0; i < __comps.Count; i++)
            {
                CompProperties props = __comps[i].props;
                if (props != null && props.compClass == typeof(T))
                {
                    actions.Add(removeAction);
                    return ThingComp_Cache<T>.compsById[thing.thingIDNumber] = __comps[i] as T;
                }
            }
            if (allowFallback)
            {
