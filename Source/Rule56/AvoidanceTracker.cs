	...
	public class AvoidanceTracker : MapComponent
	{
		...
		public override void MapComponentUpdate()
		{
			...
			for (int i = 0; i < items.Count; i++)
			{
				...
				if (item.pawn.Downed || GenTicks.TicksGame - CombatAI.Compatibility.NeedCompat.GetLastRestTick(item.pawn.needs?.rest) < 30)
				{
					continue;
				}
				TryCastProximity(item.pawn, IntVec3.Invalid);
				if (CombatAI.Compatibility.PawnPathCompatExtras.IsPawnMoving(item.pawn))
				{
					TryCastPath(item);
				}
			}
		...
		private void TryCastPath(IBucketablePawn item, PawnPath pawnPath = null)
		{
			...
			if (pawnPath?.GetNodes() == null || pawnPath.GetCurNodeIndex() <= 5)
			{
				return;
			}
			ulong         flags  = pawn.GetThingFlags();
			...
			cells.Clear();
			cells2.Clear();
			int curIndex = pawnPath.GetCurNodeIndex();
			int index = Maths.Max(curIndex - 90, 0);
			int limit = Maths.Min(index + 80, curIndex + 1);
			for (int i = index; i < limit; i++)
			{
				cells.Add(pawnPath.GetNodes()[i]);
			}
			if (cells.Count == 0)
			{
				return;
