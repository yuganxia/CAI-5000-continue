		...
		if (verbAvailable)
		{
			...
			if (!verb.IsMeleeAttack)
			{
				...
				if (projectile != null)
				{
					...
					{
							rangedSharp = Maths.Max(CombatAI.Compatibility.ProjectilePropertiesCompat.GetDamageAmount(projectile) * burstShotCount, rangedSharp);
						;
						rangedSharpAp = Maths.Max(GetArmorPenetration(projectile), rangedSharpAp);
					}
					else
					{
							rangedBlunt = Maths.Max(CombatAI.Compatibility.ProjectilePropertiesCompat.GetDamageAmount(projectile) * burstShotCount, rangedBlunt);
