...
namespace CombatAI
{
    ...
    public class CombatAI_DebugTooltipHelper : GameComponent
    {
        ...
        public override void GameComponentOnGUI()
        {
            ...
            if (!Finder.Settings.Debug || !CombatAI.Compatibility.InputCompat.AnyKey() || Find.CurrentMap == null || Current.ProgramState != ProgramState.Playing)
            {
                return;
            }
            Rect mouseRect;
        ...
        private void TryMapTooltips(Rect mouseRect, IntVec3 mouseCell)
        {
            ...
            for (int i = 0; i < mapCallbacks.Count; i++)
            {
                ...
                if (CombatAI.Compatibility.InputCompat.GetKey(callback.Second == KeyCode.None ? KeyCode.LeftShift : callback.Second))
                {
