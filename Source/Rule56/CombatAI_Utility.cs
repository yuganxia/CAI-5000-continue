    ...
    public static class CombatAI_Utility
    {
        ...
        public static IntVec3 TryGetNextDutyDest(this Pawn pawn, float maxDistFromPawn = -1)
        {
            ...
            if (!TKVCache<Tuple<int, int, IntVec3>, PawnDuty, IntVec3>.TryGet(key, out IntVec3 dutyDest, 600))
            {
                ...
                {
                    PawnPath path = CombatAI.Compatibility.PathFinderCompat.FindPath(pawn.Map, pawn.Position, pawn.mindState.duty.focus.Cell, pawn);
                    if (path != null && (path.GetNodes()?.Count ?? 0) > 0)
                    {
                        maxDistFromPawn = Mathf.Clamp(maxDistFromPawn, 5f, 64f);
                        int     i          = path.GetNodes().Count - 1;
                        float   maxDistSqr = Maths.Sqr(maxDistFromPawn);
                        IntVec3 pawnPos    = pawn.Position;
                        while (i >= 0 && path.GetNodes()[i].DistanceToSquared(pawnPos) < maxDistSqr)
                        {
                            i--;
                        }
                        dutyDest = path.GetNodes()[Maths.Max(i, 0)];
                        path.ReleaseToPool();
                    }
                }
        ...
        public static Verb TryGetAttackVerb(this Thing thing)
        {
            if (thing is Pawn pawn)
            {
                ...
                if (meleeVerbs != null)
                {
                    var cur = CombatAI.Compatibility.PawnMeleeVerbsCompat.GetCurMeleeVerb(meleeVerbs);
                    if (cur != null)
                    {
                        return cur;
                    }
