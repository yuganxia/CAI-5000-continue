...
namespace CombatAI
{
    public static class PawnPathUtility
    {
        ...
        {
            PawnPath path;
            if (!CombatAI.Compatibility.PawnPathCompatExtras.IsPawnMoving(pawn) || (path = pawn.pather?.curPath) == null || path.NodesLeftCount <= 1)
            {
                index = -1;
                return false;
            }
        ...
        public static bool TryGetSapperSubPath(this PawnPath path, Pawn pawn, List<IntVec3> store, int sightAhead, int sightStep, out IntVec3 cellBefore, out IntVec3 cellAhead, out bool enemiesAhead, out bool enemiesBefore, bool debugFlash = false)
        {
            ...
            int       i    = path.GetCurNodeIndex() - 1;
            int       num  = 0;
            IntVec3   loc  = path.GetNodes()[path.GetCurNodeIndex()];
            if (debugFlash)
            {
            ...
            if (store.Count > 0)
            {
                ...
                {
                    IntVec3 next = path.GetNodes()[i];
                    if (reader.GetVisibilityToEnemies(next) > 0 || home != null && CombatAI.Compatibility.AreaCompat.InnerGridContains(home, next))
                    {
                        enemiesAhead = true;
                        break;
                    }
                ...
                if (!enemiesAhead && home != null)
                {
                    ...
                    {
                        IntVec3 next = path.GetNodes()[i];
                        if (CombatAI.Compatibility.AreaCompat.InnerGridContains(home, next))
                        {
                            enemiesAhead = true;
                            break;
                        }
