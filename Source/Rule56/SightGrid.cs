...
namespace CombatAI
{
	public class SightGrid
	{
		...
		public SightGrid(SightTracker sightTracker, Settings.SightPerformanceSettings settings, int gridId)
		{
			...
			ticksUntilUpdate      = gridId * (this.settings.interval / 4);
			buckets               = new IBuckets<IBucketableThing>(settings.buckets);
			suRect_Combatant      = new CellRect();
			suRect_Combatant.minX = CellIndicesCompat.GetMapSizeX(map.cellIndices);
			suRect_Combatant.maxX = 0;
			suRect_Combatant.minZ = CellIndicesCompat.GetMapSizeZ(map.cellIndices);
		...
		public void FinalizeInit()
		{
			...
			{
				grid_regions.SetRegionAt(i, RegionGridCompat.RegionAt(map.regionGrid, i));
			}
			// start async actions.
			asyncActions.Start();
		}
		...
		public virtual void SightGridOptionalUpdate(bool gamePaused, bool performanceOkay)
		{
			if (gamePaused || performanceOkay)
			{
				...
				{
					Region region = RegionGridCompat.RegionAt(map.regionGrid, regionUpdateIndex);
					...
					if (thing != null && thing.Spawned)
					{
						if (thing.Position.DistanceToSquared(cell) < Maths.Sqr(SightUtility.GetSightRadius_Fast(thing)))
						{
							if (validateLoS)
							{
								...
								if (verb != null)
								{
									if (verb.IsMeleeAttack && !GenSight.LineOfSight(cell, thing.Position, map, true) || !CombatAI.Compatibility.VerbCompat.CanHitCellFromCellIgnoringRange(verb, thing.Position, cell))
									{
										continue;
									}
								}
		...
		private bool Skip(IBucketableThing item)
		{
			...
			{
				return !CombatAI.Compatibility.CompatHelpers.DormantIsAwake(item.dormant) || CombatAI.Compatibility.CompatHelpers.DormantWaitingToWakeUp(item.dormant);
			}
			if (item.Pawn != null)
			{
				return !playerAlliance && (GenTicks.TicksGame - CombatAI.Compatibility.NeedCompat.GetLastRestTick(item.Pawn.needs?.rest) <= 30 || item.Pawn.Downed);
			}
			...
			{
				return playerAlliance && (!item.TurretGun.Active || item.TurretGun.IsMannable && !CombatAI.Compatibility.CompatHelpers.IsTurretManned(item.TurretGun));
			}
		...
		private bool TryCastSight(IBucketableThing item)
		{
			...
			Action action = () =>
			{
				...
				Action<IntVec3, int, int, float> setAction = (cell, carry, dist, coverRating) =>
				{
					...
					{
						// Skip spotting if this cell is currently fogged for the player.
						if (fogComp != null && fogComp.IsFogged(cell))
						{
							return;
						}
				...
				flooder.Flood(origin, node =>
				{
					...
					if (scanForEnemies)
					{
						// Skip nearby spotting if node is fogged for player
						if (fogComp != null && fogComp.IsFogged(node.cell))
						{
							return;
						}
