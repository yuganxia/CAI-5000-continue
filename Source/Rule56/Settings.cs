...
namespace CombatAI
{
    public class Settings : ModSettings
    {
        ...
        // When true and FogOfWar_Enabled is true, disable fog only on the player's current map
        public bool  FogOfWar_DisableOnPlayerMap = false;
        ...
        public override void ExposeData()
        {
	        ...
	        if (Scribe.mode == LoadSaveMode.Saving)
	        {
                ...
                {
                    var k = pair.Key.Item1.defName + "_" + (pair.Key.Item2?.defName ?? string.Empty);
                    if (!_raceSettings.ContainsKey(k)) _raceSettings[k] = pair.Value;
                }
	        }
