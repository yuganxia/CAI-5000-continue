    ...
    public static class MapPawns_Patch
    {
        ...
        public static class MapPawns_RegisterPawn_Patch
        {
            public static void Prefix(MapPawns __instance, Pawn p)
            {
                ...
                if (map != null)
                {
                    map.GetComp_Fast<SightTracker>()?.Register(p);
                    map.GetComp_Fast<AvoidanceTracker>()?.Register(p);
                }
            }
        }
        ...
        public static class MapPawns_DeRegisterPawn_Patch
        {
            public static void Prefix(MapPawns __instance, Pawn p)
            {
                ...
                if (map != null)
                {
                    map.GetComp_Fast<SightTracker>()?.DeRegister(p);
                    map.GetComp_Fast<AvoidanceTracker>()?.DeRegister(p);
                }
