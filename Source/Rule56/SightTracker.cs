...
//		                                            List<Thing> store = new List<Thing>();
        ...
        public void Register(Thing thing)
        {
            ...
            // Ensure wall grid knows about this thing (keeps fill values up to date)
            try
            {
                var wg = map.GetComp_Fast<WallGrid>();
                wg?.RecalculateCell(thing.Position, thing);
            }
            catch (Exception) { }
        }
        ...
        public void DeRegister(Thing thing)
        {
            ...
            {
                var wg = map.GetComp_Fast<WallGrid>();
                wg?.RecalculateCell(thing.Position, null);
            }
            catch (Exception) { }
        }
        ...
        public class SightReader
        {
            ...
            private readonly bool readerIsPlayer;
            ...
            public SightReader(SightTracker tracker, SightGrid[] friendlies, SightGrid[] hostiles, SightGrid[] neutrals)
            {
                ...
                for (int i = 0; i < friendlies.Length; i++)
                {
                    if (friendlies[i] != null && friendlies[i].playerAlliance)
                    {
                        _isPlayer = true;
                        break;
                    }
                }
                readerIsPlayer = _isPlayer;
            }
            ...
            {
                // If this reader is for player/allied forces, respect fog of war.
                if (readerIsPlayer && Tracker?.fogGrid != null)
                {
                    if (Tracker.fogGrid.Get(index) <= 1e-3f)
                    {
                        return 0ul;
                    }
                }
            ...
            {
            // Respect fog for player readers
            if (readerIsPlayer && Tracker?.fogGrid != null)
            {
                if (Tracker.fogGrid.Get(index) <= 1e-3f)
                {
